...
MODULE_PATH = {
    ...
    "ingestionpipeline": "services.ingestionPipelines"
}
...
class OpenMetadata(
    ...
    def update_file_name(create: Type[C], file_name: str) -> str:
        ...
        if "ingestionpipeline" in create.__name__.lower():
            return file_name.replace("ingestionpipeline", "ingestionPipeline")
    ...
    def get_entity_from_create(self, create: Type[C]) -> Type[T]:
        ...
        cp1 = self.class_root,
        cp2 = self.entity_path if not file_name.startswith("test") else None
        cp3 = self.get_module_path(create)
