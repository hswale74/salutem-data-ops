...
# -----------------------------------------------------------------------------------------------------
    def __init__(self, domain='login', api_version='56.0') -> None:
        ...
        self.__access_token = login_response.json()["access_token"] # Fetching Access Token
        self.__instance_url = login_response.json()["instance_url"] # Fetching Instance Url
...
# -----------------------------------------------------------------------------------------------------
    def create_job(self, operation :str, object_name :str, line_ending="CRLF") -> None:
        ...
        job_url = f'{self.__instance_url}/services/data/v{self.api_version}/jobs/ingest/'
        ...
        job_response = requests.post(job_url, headers=headers, data=json.dumps(job_data))
...
#------------------------------------------------------------------------------------------------------        
    def upload_complete(self) -> bool:
        ...
        response = requests.patch(upload_complete_url, headers=headers, data=json.dumps({'state':'UploadComplete'}))
...
#--------------------------------------------------------------------------------------------------------
    def store_results(self, object_name :str, operation_name :str, environment :str) -> None:
        """
        ...
        operation_folder_path = f'./files/cud_objects/{environment}/{object_name}/{operation_name}'
        failed_records_folder_path = f'./files/cud_objects/{environment}/{object_name}/{operation_name}/failed'
        ...
        if response.status_code == 200:
            if len(response.text)!=0:
                ...
                with open(f'{successful_records_folder_path}/success_{datetime.now().strftime("%Y-%m-%d %H-%M-%S")}.csv', 'w', encoding = 'utf-8') as file_handler:
        ...
        if response.status_code == 200:
            if len(response.text)!=0:
                ...
                with open(f'{failed_records_folder_path}/failed_{datetime.now().strftime("%Y-%m-%d %H-%M-%S")}.csv', 'w', encoding = 'utf-8') as file_handler:
                    file_handler.write(records_data)
        #-----------------------------------------------------
        ...
        if response.status_code == 200:
            if len(response.text)!=0:
                ...
                with open(f'{unprocessed_records_folder_path}/unprocessed_{datetime.now().strftime("%Y-%m-%d %H-%M-%S")}.csv', 'w', encoding = 'utf-8') as file_handler:
                    file_handler.write(records_data)
        #-----------------------------------------------------
#-----------------------------------------------------------------------------------------------------

#-----------------------------------------------------------------------------------------------------
    def cud(self, file_path :str, object_name :str, operation_name :str, environment :str):
        """
            ...
            if(self.upload_complete()):
                while True:
                    ...
                    if job_status_response.status_code == 200:
                        ...
                        successful_records = job_status['numberRecordsProcessed'] - job_status['numberRecordsFailed']
                    ...
                    if final_status_response.status_code == 200:
                        ...
                        print(f"Total Records Processed :- {final_status['numberRecordsProcessed']}")
                        print(f"Successful Records :- {final_status['numberRecordsProcessed'] - final_status['numberRecordsFailed']}")
                        print(f"Failed Records :- {final_status['numberRecordsFailed']}")
                        print(f"Total Processing Time :- {final_status['totalProcessingTime']}" )
                        print(f"Api Processing Time :- {final_status['apiActiveProcessingTime']}")
...
def main():
    ...
    environment = "sit"
    SalesforceBulk2(domain='test').cud(csv_file_path, object_name, 'update', environment)
# -----------------------------------------------------------------------------------------------------
